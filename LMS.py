...
#arithmos eterion
companies_length = len(companies_odds) #odds

#epanelave (iteration) gia kathe match
for match in Match_List:
    goals_H = match[9]   #We get how many goals were scored by home team
    goals_A = match[10]  #We get how many goals were scored by away team
    result = extfunc.result(goals_H,goals_A)          #we see result as H , D , A depending on the scord diffrence
    Match_Results_List.append([int(match[6]),result])   #Match_Results_List stores the match result for each match(match_id)
...
for company,company_score in enumerate(scores): #edo ginete tuple unpacking enumerated scores
...
for c,company in enumerate(Weight): #edo ginete tuple unpacking enumerated Weights
